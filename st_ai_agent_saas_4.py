...
if st.button("Submit"):
    if "add" in selected_command:
        key, value = selected_command.split("add ")[1].split(" as ")
        st.write(agent.add_data(key, value))
    elif "get" in selected_command:
        key = selected_command.split("get ")[1]
        st.write(agent.get_data(key))
    elif "update" in selected_command:
        key, value = selected_command.split("update ")[1].split(" to ")
        st.write(agent.update_data(key, value))
    elif "delete" in selected_command:
        key = selected_command.split("delete ")[1]
        st.write(agent.delete_data(key))
    elif "sum" in selected_command:
        keys = selected_command.split("sum ")[1].split(" and ")
        st.write(agent.sum_data(keys))
    elif "multiply" in selected_command:
        keys = selected_command.split("multiply ")[1].split(" and ")
        st.write(agent.multiply_data(keys))
    elif "formula" in selected_command:
        formula = selected_command.split("formula ")[1]
        st.write(agent.evaluate_formula(formula))
    elif "list all" in selected_command:
        st.write(agent.list_all())
    elif "save csv" in selected_command:
